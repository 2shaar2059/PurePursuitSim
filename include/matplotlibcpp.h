...
template<typename Numeric>
bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const char* label)
{
    assert(x.size() == y.size());
    detail::_interpreter::get();
    ...
    // construct keyword args
    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "label", PyString_FromString(label));
...
template<typename NumericX, typename NumericY, typename NumericZ, typename NumericU, typename NumericV, typename NumericW>
bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, const std::vector<NumericU>& u, const std::vector<NumericV>& v, const std::vector<NumericW>& w, const std::map<std::string, std::string>& keywords = {})
{
    assert(x.size() == y.size() && y.size() == z.size() && z.size() == u.size() && u.size() == v.size() && v.size() == w.size());
    ...
    PyTuple_SetItem(plot_args, 4, varray);
    PyTuple_SetItem(plot_args, 5, warray);
